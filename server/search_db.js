...
async function run() {
    const { data } = await supabase.from('company_articles').select('seo_title').ilike('seo_title', '%Learning%');
    console.log("company_articles:", data);
    const { data: d2 } = await supabase.from('articles').select('seo_title').ilike('seo_title', '%Learning%');
    console.log("articles:", d2);
}
run();
