"""
routes.py — FastAPI router for the Graphic Agent API.

Two invocation modes are supported per endpoint:

  1. Agent mode  (use_agent=True, default):
       Routes the full request through the LangChain decision-making agent.
       The agent picks which tools to call and in what order.

  2. Direct mode (use_agent=False):
       Calls the service layer directly — same deterministic flow as before
       the refactor. Provides full backward compatibility.

The `session_id` query parameter (optional) scopes agent memory per user/session.
"""
