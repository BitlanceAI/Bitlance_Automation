            ...
            <div className="max-w-5xl mx-auto space-y-8">
                <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
                    <div>
                        ...
                        <p className="text-sm font-mono text-teal-50/70 mt-1 uppercase tracking-widest">Start with a proven framework for high-engagement posts</p>
                    </div>
                </div>
                ...
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="w-full bg-[#111] border border-white/10 text-white rounded-[2px] pl-11 pr-4 py-3 text-[13px] font-mono tracking-widest uppercase focus:outline-none focus:border-[#26cece] placeholder:text-white/40 transition-colors"
                    />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {templates.map((tpl, i) => (
                        <div key={i} className="bg-white/5 border border-white/10 hover:border-[#26cece]/50 rounded-[2px] p-6 hover:-translate-y-1 hover:shadow-[0_8px_24px_rgba(38,206,206,0.15)] transition-all cursor-pointer group flex flex-col">
                            <div className="flex items-center justify-between mb-4">
                                <div className="w-10 h-10 bg-[#26cece]/10 rounded-[2px] flex items-center justify-center border border-white/10 group-hover:border-[#26cece]/30 transition-colors">
                                    <tpl.icon className="w-5 h-5 text-[#26cece]" />
                                </div>
                                <span className="text-[10px] font-mono uppercase tracking-widest text-white/50 bg-black/20 px-2 py-1 rounded-[2px] border border-white/10">{tpl.category}</span>
                            </div>
                            <h3 className="text-[15px] font-bold text-white font-['Space_Grotesk'] uppercase tracking-tight mb-2">{tpl.title}</h3>
                            <p className="text-[12px] font-sans text-white/60 flex-1">{tpl.desc}</p>
